...
package com.idrive.galleryphotos.viewmodels
...
class MainActivityViewModel(application: Application) : AndroidViewModel(application) {
    ...
    fun showLocationToastMsg(image: MediaStoreImage) {
        viewModelScope.launch {
            val geocoder = Geocoder(getApplication(), Locale.getDefault())
            val latLong = getLocationData(image.contentUri)
            val location =
                geocoder.getFromLocation(latLong[0].toDouble(), latLong[1].toDouble(), 3)
            var locationText: String = "";
            for (loc in location) {
                locationText += loc.locality + ", "
            }
            locationText.dropLast(2)
            Toast.makeText(getApplication(), "Locations : $locationText", Toast.LENGTH_LONG).show()
        }
    }

    fun getLocationData(uri: Uri): FloatArray {
        return try {
            if (isQPlus()) {
                getLocationDataForAboveQ(uri)
            } else {
                getLocationDataForBelowQ(uri)
            }
        } catch (e: Exception) {
    ...
    private fun getLocationDataForBelowQ(uri: Uri): FloatArray {
        ...
        cursor?.use {
            try {
                if (it.moveToFirst()) {
                    val latitudeColumn = 0
                    val longitudeColumn = 1

                    val latLong = FloatArray(2)

                    latLong[0] = it.getFloat(latitudeColumn)
                    latLong[1] = it.getFloat(longitudeColumn)
    ...
    private fun getLocationDataForAboveQ(uri: Uri): FloatArray {
        val latLong = FloatArray(2)
        try {
            if (isQPlus()) {
                val originalUri = MediaStore.setRequireOriginal(uri)

                getApplication<Application>().contentResolver.openInputStream(originalUri)?.use {
